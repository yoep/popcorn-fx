use crate::torrent::peer::PeerHandle;
use crate::torrent::{PartIndex, PieceIndex, PiecePart};
...
impl PendingRequest {
    pub fn new(piece: PieceIndex, parts: Vec<PiecePart>) -> Self {
        Self {
            inner: Arc::new(InnerPendingRequest {
                ...
                blacklisted: std::sync::RwLock::new(Vec::with_capacity(0)),
    ...
    /// Get the peer that has requested the given part of the pending request.
    /// It returns the peer handle if the part has been requested, else None.
    pub fn part_requester(&self, part: PartIndex) -> Option<PeerHandle> {
        self.inner
            .pending_requests
            .read()
            .unwrap()
    ...
    /// Check if the given peer has been blacklisted for this request.
    /// If a peer is blacklisted, it was rejected for this request, and we should not retry requesting it from the same peer.
    pub fn is_blacklisted(&self, peer: &PeerHandle) -> bool {
        let mutex = self.inner.blacklisted.read().unwrap();
        mutex.contains(&peer)
    }

    /// Blacklist a certain peer for this pending request.
    /// This is used when a peer has rejected a part of the pending request and we should not retry requesting it from the same peer.
    pub fn blacklist(&self, peer: PeerHandle) {
        let mut mutex = self.inner.blacklisted.write().unwrap();
        mutex.push(peer);
    }
...
struct InnerPendingRequest {
    ...
    /// The blacklisted peer handles from which we should not try to download the piece
    blacklisted: std::sync::RwLock<Vec<PeerHandle>>,
...
impl PendingRequestBuffer {
    ...
    /// # Returns
    ///
    /// Returns true if the piece has been completed.
    pub fn update_request_part_completed(&mut self, piece: PieceIndex, part: PartIndex) -> bool {
        let is_piece_completed: bool;

        if let Some(request) = self.requests.iter().find(|e| e.piece() == piece) {
            request.mark_part_as_completed(part);
            is_piece_completed = request.is_completed();
        } else {
            warn!("Piece {} does not have a pending request", piece);
            return false;
        }
    ...
    /// Blacklist a peer for a pending request and release the part index as being processed by the peer.
    /// This will allow the part to be downloaded from another peer.
    pub fn blacklist_and_retry(
        &self,
        request: &PendingRequest,
        part: &PartIndex,
        peer: PeerHandle,
    ) {
        request.blacklist(peer);
