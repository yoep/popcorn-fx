...
impl Discovery for ChromecastDiscovery {
    ...
    async fn start_discovery(&self) -> crate::Result<()> {
        ...
        if state != DiscoveryState::Running {
            ...
            let receiver = self.inner.service_daemon.browse(SERVICE_TYPE)
    ...
    fn stop_discovery(&self) -> crate::Result<()> {
        ...
        if state == DiscoveryState::Running {
            self.inner.service_daemon.stop_browse(SERVICE_TYPE)
...
impl InnerChromecastDiscovery {
    ...
    async fn handle_event(&self, event: ServiceEvent) {
        if let ServiceEvent::ServiceResolved(info) = event {
            ...
            if let Some(addr) = info.get_addresses().into_iter()
    ...
    async fn register_device<S: Into<String>>(&self, info: ServiceInfo, addr: S, port: u16) -> chromecast::Result<()> {
