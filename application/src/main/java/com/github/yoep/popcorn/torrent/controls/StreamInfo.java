...
public class StreamInfo extends Icon {
     ...
     */
    public void update(StreamStatus status) {
    ...
    private class StreamPopup extends PopupControl {
         ...
         * @param status The update information to display.
         */
        void updateInfo(StreamStatus status) {
    ...
    private class StreamPopupSkin implements Skin<StreamPopup> {
        ...
        void updateInfo(StreamStatus status) {
            ...
            String downloaded = MessageFormat.format("{0} ({1}%)", StreamStatus.toDisplaySize(status.getDownloaded()), progress);

            downloadValue.setText(StreamStatus.toDisplaySize(status.getDownloadSpeed()) + "/s");
            uploadValue.setText(StreamStatus.toDisplaySize(status.getUploadSpeed()) + "/s");
