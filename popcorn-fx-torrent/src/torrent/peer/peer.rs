...
use crate::torrent::peer::peer_request_buffer::PeerRequestBuffer;
...
use crate::torrent::{InfoHash, PieceIndex, PiecePart, Torrent, TorrentInfo, TorrentMetadata};
use bit_vec::BitVec;
...
impl Peer {
    pub async fn new_outbound(
        ...
        select! {
            ...
            stream = TcpStream::connect(&addr) => Self::process_connection_stream(peer_id, addr, stream?, ConnectionType::Outbound, torrent, protocol_extensions, extensions, runtime).await
        }
    }
    ...
    pub async fn new_inbound(
        ...
        select! {
            ...
            result = Self::process_connection_stream(peer_id, addr, stream, ConnectionType::Inbound, torrent, protocol_extensions, extensions, runtime) => result
        }
    }
...
impl InnerPeer {
    ...
    fn invoke_event(&self, event: PeerEvent) {
        if let Err(e) = self.extension_event_sender.send(event.clone()) {
            warn!("Failed to send extensions event, {}", e)
        }
