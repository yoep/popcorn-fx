...
impl File {
    ...
    ///     let (offset, range) = file.byte_range(&piece).unwrap();
    ///
    ...
    pub fn byte_range(&self, piece: &Piece) -> Option<(usize, Range<usize>)> {
        let file_end = self.offset + self.length;
        let piece_end = piece.offset + piece.length;
        let start = self.offset.max(piece.offset);
        let end = file_end.min(piece_end);
        let file_offset = piece.offset.checked_sub(self.offset).unwrap_or(0);

        if start < end {
            Some((file_offset, start..end))
        } else {
            None
        }
    }
...
mod tests {
    ...
    fn test_byte_range() {
        let piece = Piece::new(InfoHash::default(), 0, 0, 64);
        let file = new_file(0, 128);
        let result = file.byte_range(&piece);
