    ...
    async fn create_pending_requests(&self, torrent: &TorrentContext) {
        ...
        if wanted_pieces.len() > 0 {
            ...
            let new_pending_requests: Vec<PendingRequest> = wanted_pieces
                ...
                .sorted_by(|a, b| {
                    // order by highest availability
                    b.availability.cmp(&a.availability)
                })
...
mod tests {
    ...
    use crate::torrents::operations::TorrentPiecesOperation;
    use crate::torrents::{PiecePriority, Torrent, TorrentInfo};
